
<div align="center">

<!-- Header Banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Hi,%20I'm%20Yumi%20👋&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32"/>

<!-- Typing Animation -->
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=6B8AF7&center=true&vCenter=true&random=false&width=500&lines=AI+Engineer+%26+Educator;RAG+Systems+%7C+LLM+Applications;%22%EB%B3%B5%EC%9E%A1%ED%95%9C+%EA%B2%83%EC%9D%84+%EB%8B%A8%EC%88%9C%ED%95%98%EA%B2%8C%22" alt="Typing SVG" /></a>

</div>

---

## 🙋‍♀️ About Me

```yaml
name: Yumi Park
location: Seoul, Korea
education: M.S. in Statistics
current_role: AI Engineer & Educator

interests:
  - RAG Systems & Document AI
  - Generative Models (VQGAN, MaskGIT)
  - Making complex tech simple

motto: "복잡한 것을 단순하게, 어려운 것을 쉽게"
```

---

## 🔧 What I Do

🤖 **RAG 시스템 개발** — 부동산 리포트 챗봇, 건설 규정 AI Agent 등 문서 기반 AI 시스템을 설계하고 구축합니다.

📚 **AI 교육** — 삼성물산 AI Academy에서 임직원 대상 생성형 AI 교육을 진행하며, 실습 중심의 커리큘럼을 개발했습니다.

🎨 **Generative AI 실험** — VQGAN/MaskGIT 기반 이미지 생성 등 최신 모델을 탐구하고 PoC를 구축합니다.

---

## 🛠 Tech Stack

<div align="center">

#### 💬 LLM / RAG
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![FAISS](https://img.shields.io/badge/FAISS-00A4EF?style=for-the-badge&logo=meta&logoColor=white)

#### 🧠 ML / DL
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![HuggingFace](https://img.shields.io/badge/🤗%20Transformers-FFD21E?style=for-the-badge)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)

#### ⚙️ Backend
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

#### 🚀 Infra & Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)

</div>

---

## 📂 Featured Projects

### 🎫 Ticketmon
> 대규모 동시접속 티켓팅 시스템

`Spring Boot` `React` `Redis` `Docker`

- Redis 기반 대기열 시스템 및 분산 락 구현
- AI 리뷰 요약 기능 (OpenAI API)
- GitHub Actions CI/CD 파이프라인

[![Backend](https://img.shields.io/badge/Backend-181717?style=flat-square&logo=github)](https://github.com/LimPark996/Ticketing-Website_BE)
[![Frontend](https://img.shields.io/badge/Frontend-181717?style=flat-square&logo=github)](https://github.com/LimPark996/Ticketing-Website_FE)

---

### 🔍 Term Search System
> 지능형 하이브리드 용어 검색 시스템

`Python` `FAISS` `ColBERT` `OpenAI API`

- GPT 기반 쿼리 전처리 + FAISS/ColBERT 하이브리드 검색
- FAISS 벡터 유사도 + ColBERT 토큰 매칭 결합
- Circuit Breaker, Rate Limiting, Fallback 장애 대응

[![Repo](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github)](https://github.com/LimPark996/term-search-system-ver1)

---

### 🏠 Real Estate RAG Chatbot
> 부동산 리포트 기반 AI 챗봇 · 삼성물산 AI Academy 교육 프로젝트

`Python` `FAISS` `Streamlit` `OpenAI API`

- PDF 텍스트/표/그래프 통합 처리
- FAISS 벡터 검색 + BM25 키워드 검색 결합

[![Repo](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github)](https://github.com/LimPark996/cntchatbot_pjt1)

---

### 🔩 Metal Defect Synthesis
> 제조업 품질검사용 결함 이미지 생성 PoC

`VQGAN` `MaskGIT` `Gradio` `HuggingFace`

- LlamaGen VQGAN + Halton-MaskGIT 파이프라인
- 6종 금속 결함 클래스 inpainting 데모

[![Repo](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github)](https://github.com/LimPark996/metal-defect-synthesis)

---

## 📫 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/%EC%9C%A0%EB%AF%B8-%EB%B0%95-4a766828a/)
[![HuggingFace](https://img.shields.io/badge/🤗%20HuggingFace-FFD21E?style=for-the-badge)](https://huggingface.co/Yumi-Park996)
</div>

---

<!-- Footer -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer"/>
